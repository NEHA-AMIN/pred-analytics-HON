...
def main():
    ...
    print(f"  Time period: {dataset_config['start_date']} to " f"{dataset_config['num_days']} days")
    ...
    catalog_generator = ItemCatalogGenerator(config=config["item_catalog"], random_seed=random_seed)
    ...
    print(
        f"  Items with discount: {catalog_stats['items_with_discount']} "
    ...
    user_simulator = UserBehaviorSimulator(config=config["user_personas"], random_seed=random_seed)

    users_df = user_simulator.generate_users(num_users=dataset_config["num_users"])
    ...
    clickstream_generator = ClickstreamGenerator(
        users_df=users_df, catalog_df=catalog_df, config=config, random_seed=random_seed
    )
