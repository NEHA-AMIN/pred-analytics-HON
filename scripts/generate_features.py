...
def generate_features_for_split(
    events_df: pd.DataFrame, catalog_df: pd.DataFrame, split_name: str, output_dir: Path
):
    """
    ...
    # Extract features
    features_df = engineer.create_training_examples(events_df=events_df, catalog_df=catalog_df)
...
def main():
    ...
    for split in splits:
        ...
        features_df = generate_features_for_split(
            events_df=events_df, catalog_df=catalog_df, split_name=split, output_dir=output_dir
        )
