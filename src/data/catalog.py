...
class ItemCatalogGenerator:
    ...
    def generate_catalog(self) -> pd.DataFrame:
        ...
        for category_config in self.config["categories"]:
            ...
            category_items = self._generate_category_items(
                category=category, start_item_id=item_id_counter
            )
        ...
        print(
            f"✅ Generated catalog: {len(catalog_df)} items across "
            f"{len(self.config['categories'])} categories"
        )

        return catalog_df

    def _generate_category_items(self, category: Category, start_item_id: int) -> pd.DataFrame:
        """
        ...
        if has_discount.any():
            ...
            discount_pcts[has_discount] = self.rng.uniform(min_disc, max_disc, has_discount.sum())
    ...
    def get_catalog_stats(self, catalog_df: pd.DataFrame) -> Dict:
        ...
        stats = {
            ...
            "price_range": [catalog_df["current_price"].min(), catalog_df["current_price"].max()],
...
def main():
    ...
    generator = ItemCatalogGenerator(
        config=config["item_catalog"], random_seed=config["dataset"]["random_seed"]
    )
    ...
    print(
        f"  Items with discount: {stats['items_with_discount']} " f"({stats['discount_rate']:.1%})"
    )
