...
class UserBehaviorSimulator:
    ...
    def generate_users(self, num_users: int) -> pd.DataFrame:
        ...
        personas = self.rng.choice([p.value for p in UserPersona], size=num_users, p=persona_probs)
        ...
        print(
            f"   Browsers: {(users_df['persona'] == 'browser').sum()} "
            f"({(users_df['persona'] == 'browser').mean():.1%})"
        )
        print(
            f"   Researchers: {(users_df['persona'] == 'researcher').sum()} "
            f"({(users_df['persona'] == 'researcher').mean():.1%})"
        )
        print(
            f"   Buyers: {(users_df['persona'] == 'buyer').sum()} "
    ...
    def simulate_session_behavior(
        self, persona: UserPersona, num_items_viewed: int
    ) -> Dict[str, List[int]]:
        """
        ...
        if cart_indices:
            ...
            purchased_indices = [
                cart_indices[i] for i, purchased in enumerate(purchase_mask) if purchased
            ]
...
def main():
    ...
    simulator = UserBehaviorSimulator(
        config=config["user_personas"], random_seed=config["dataset"]["random_seed"]
    )

    users_df = simulator.generate_users(num_users=config["dataset"]["num_users"])
